...
import { Mail, Lock, Eye, EyeOff, GitFork, Star, ShieldCheck, Moon, Sun, ArrowLeft } from "lucide-react";
...
export default function SignInPage() {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.error) {
        setError(result.error.message || "Could not find your account.");
        setIsLoading(false);
        return;
      }
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signIn.status === "complete") {
        ...
        setError(`Unable to complete sign in. Status: ${signIn.status}`);
      }
    } catch (err: any) {
      setError("An unexpected error occurred during sign in.");
    } finally {
        ...
        <section className="lg:col-span-5 flex justify-center items-center py-6">
          <div className="w-full max-w-md bg-white dark:bg-[#060b06] rounded-[32px] border border-slate-200/50 dark:border-[#a8ff35]/20 p-8 lg:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.02)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.5)] transition-all">
            ...
            {step === 1 ? (
              ...
              <>
                {/* Step 2: Password Form (Google style) */}
                <div className="mb-6 flex items-center gap-3">
                  <button
                    ...
                    className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-900 transition-all"
                    title="Change Email"
                  >
                    <ArrowLeft className="w-4 h-4" />
                  </button>
                  <div>
                    <h2 className="font-retro text-2xl font-bold text-green-600 dark:text-[#a8ff35]">
                      Enter password
                    </h2>
                ...
                <form onSubmit={handlePasswordSubmit} className="space-y-6">
                  ...
                  <div className="flex items-center justify-between text-xs font-share-mono">
                    <label className="flex items-center gap-2 cursor-pointer select-none text-slate-600 dark:text-slate-400">
                      <input
                        type="checkbox"
                        checked={rememberMe}
                        onChange={(e) => setRememberMe(e.target.checked)}
                        className="w-4 h-4 rounded border-slate-300 dark:border-[#a8ff35]/25 accent-green-600 dark:accent-[#a8ff35] bg-slate-50 dark:bg-[#030503]"
                      />
                      Remember me
                    </label>
                    <span className="text-green-600 dark:text-[#a8ff35] hover:underline cursor-pointer">
                      Forgot password?
                    </span>
                  </div>
            ...
            <div className="mt-6 text-center text-xs font-share-mono text-slate-500 dark:text-slate-400">
              New here?{" "}
              <Link href="/sign-up" className="text-green-600 dark:text-[#a8ff35] hover:underline font-semibold">
                Create an account
              </Link>
            </div>

            {/* Bottom tree slogan */}
            <div className="mt-6 flex items-center justify-center gap-1.5 text-[10px] font-share-mono text-slate-400 dark:text-slate-500 uppercase tracking-widest">
